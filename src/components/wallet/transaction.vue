			...
			<van-tab title="交易记录">
				<van-pull-refresh v-model="isRefreshing" @refresh="onRefresh">
					<van-list v-model="loading" :finished="finished" :offset="100" finished-text="没有更多了" @load="onLoad">
						<van-cell v-for="(item, index) in list" class="copy" :key="index" :class="{ sent: item.from_did === metadata.did, receive: item.from_did !== metadata.did }"
						 :data-clipboard-text="item.from_did === metadata.did ? item.to_did : item.from_did" :center="true" :value="item.balance | money"
