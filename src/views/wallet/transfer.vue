		...
		<ValidationObserver v-slot="{ invalid }" ref="form">
			<van-cell-group title="收款账号" :border="false">
				<ValidationProvider v-slot="{ errors }" rules="required" name="receiver">
					<van-field v-model="transferForm.receiver" :error-message="errors[0]" placeholder="请输入收款账号">
						<van-uploader slot="right-icon" class="upload" :before-read="beforeUpload" :preview-image="false" :after-read="handleSuccess">
							选择头像
						</van-uploader>
					</van-field>
				</ValidationProvider>
			...
			<van-cell-group title="转账数量" :border="false">
				<ValidationProvider v-slot="{ errors }" rules="required|min_value:0.01" name="amount">
					<van-field v-model="transferForm.amount" type="number" :error-message="errors[0]" placeholder="请输入转账数量" />
			...
			<van-cell-group title="Memo" :border="false">
				<ValidationProvider v-slot="{ errors }" name="memo">
					<van-field v-model="transferForm.memo" :error-message="errors[0]" placeholder="请输入memo" />
				</ValidationProvider>
			</van-cell-group>
			<van-button type="primary" square size="large" :disabled="invalid" @click="handleSubmit">确认转账</van-button>
		</ValidationObserver>
	</div>
</template>
<script>
