			...
			<ValidationObserver v-slot="{ invalid }" ref="form">
				<van-cell-group :title="`绑定${addressType}账号`" :border="false">
					<ValidationProvider v-slot="{ errors }" rules="required" name="address">
						<van-field v-model="address" :error-message="errors[0]" :placeholder="placeholder" />
					</ValidationProvider>
				</van-cell-group>
				<van-button type="primary" square class="btn-submit" size="large" :disabled="invalid" @click="handleSubmit">确认更新</van-button>
			</ValidationObserver>
		</div>
	</div>
</template>
